...
/**
 * Handles downloading and managing the Gemma3-1B-IT LLM model.
...
class GemmaModelDownloader(private val context: Context) {
    ...
    companion object {
        ...
        private const val GEMMA_MODEL_URL =
            "https://github.com/divyesh11/squim-models/releases/download/v1.0.7/gemma3_1b_int8.task"
        private const val MODEL_FILE_NAME = "Gemma3-1B-IT.task"
    ...
    private val okHttpClient: OkHttpClient by lazy {
        ...
        OkHttpClient.Builder()
            ...
            .readTimeout(600, TimeUnit.SECONDS) // 10 min for large file download
    ...
    suspend fun downloadModelIfNeeded(onProgress: ((Float) -> Unit)? = null): File {
        ...
        if (isModelDownloaded()) {
            VoiceLogger.d(TAG, "Model already downloaded: ${modelFile.absolutePath}")
            onProgress?.invoke(1.0f)
            return modelFile
        }

        VoiceLogger.d(TAG, "Starting model download from HuggingFace...")
        ...
        try {
            ...
            if (!response.isSuccessful) {
                throw RuntimeException("Failed to download model: HTTP ${response.code}")
            }
            ...
            VoiceLogger.d(TAG, "Model size: ${contentLength / (1024 * 1024)} MB")
            ...
            VoiceLogger.d(TAG, "Model downloaded successfully: ${modelFile.absolutePath}")
            VoiceLogger.d(TAG, "Model size: ${modelFile.length() / (1024 * 1024)} MB")
            ...
            throw RuntimeException("Failed to download Gemma model: ${e.message}", e)
        }
    }
