...
internal class VToRxRepository(
    ...
    fun fetchVoice2RxTransactionResult(sessionId: String) {
        CoroutineScope(Dispatchers.IO).launch {
            val successStates = setOf(
                Voice2RxStatus.SUCCESS,
                Voice2RxStatus.PARTIAL_COMPLETED
            )
            var retryCount = 0
                            ...
                            EventLog.Info(
                                ...
                                params = JSONObject(
                                    mapOf(
                                        ...
                                        "response_status" to "success",
                                        "status" to (response.body.status?.name ?: "UNKNOWN"),
                                    )
                                )
                            )
                        )
                        if (response.body.status == Voice2RxStatus.IN_PROGRESS) {
                            VoiceLogger.d("Voice2Rx", "Transaction is still in progress")
                            continue
                        }
                        if (response.body.status in successStates) {
                            updateSessionUploadStage(
                                sessionId = sessionId,
    ...
    suspend fun getAllFiles(sessionId: String): List<VoiceFile> {
        return withContext(Dispatchers.IO) {
            try {
                vToRxDatabase.getVoice2RxDao().getAllFiles(sessionId = sessionId)
            } catch (e: Exception) {
                EventLog.Info(
                    code = EventCode.VOICE2RX_SESSION_ERROR,
                    params = JSONObject(
    ...
    fun getSessionInfoAsFlow(sessionId: String): Flow<VToRxSession> {
        return vToRxDatabase.getVoice2RxDao().getSessionAsFlow(sessionId = sessionId)
    }
